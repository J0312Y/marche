...
class Wallet extends Vendor_Controller
{
    ...
    public function withdraw() {
        ...
        if (!$data) return;
        $result = $this->Wallet_model->debit($this->user_id, $data['amount'], 'Retrait vendeur');
        if ($result === false) return json_error('Solde insuffisant', 422);
        $this->load->library('Payment_lib');
        $this->payment_lib->transfer($data['method'], $data['amount'], $data['phone_number'], 'VWDR-' . $this->user_id);
