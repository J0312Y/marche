...
function LoginScr({onDone,onSocial}){
  ...
  return(
    <div className="auth">
      ...
      <button className="btn-primary" onClick={()=>{const digits=phone.replace(/\s/g,"");if(digits.length!==9){setPhoneErr("Le numéro doit contenir 9 chiffres après +242");return}setPhoneErr("");onDone()}}>Continuer</button>
      ...
      <p style={{textAlign:"center",fontSize:12,color:"var(--muted)",marginTop:24}}>En continuant, vous acceptez nos <b style={{color:"var(--sub)",cursor:"pointer",textDecoration:"underline"}} onClick={()=>setLegal("terms")}>Conditions</b> et <b style={{color:"var(--sub)",cursor:"pointer",textDecoration:"underline"}} onClick={()=>setLegal("privacy")}>Politique de confidentialité</b></p>
    </div>
  );
}
