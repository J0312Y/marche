  ...
  return(<>
    ...
    <div className="scr" style={{padding:16}}>
      ...
      {dl.payment==="cash"&&<div style={{padding:12,background:"rgba(245,158,11,0.08)",border:"1px solid rgba(245,158,11,0.2)",borderRadius:14,marginBottom:12,display:"flex",alignItems:"center",gap:10}}>
        <span style={{fontSize:24}}><Icon name="wallet" size={18}/></span>
      ...
      <div style={{padding:16,background:step<2?"rgba(249,115,22,0.04)":"rgba(16,185,129,0.04)",border:"1px solid "+(step<2?"rgba(249,115,22,0.15)":"rgba(16,185,129,0.15)"),borderRadius:16,marginBottom:14}}>
        ...
        {step<2?<>
          ...
          <div style={{display:"flex",gap:8,marginTop:10}}>
            <button style={{flex:1,padding:10,borderRadius:10,border:"none",background:"#F97316",color:"#fff",fontSize:12,fontWeight:600,cursor:"pointer",fontFamily:"inherit"}} onClick={()=>go("drChatVendor",dl)}><Icon name="chat" size={16}/>{" "}Commerce</button>
            <button style={{width:42,padding:10,borderRadius:10,border:"none",background:"#F59E0B",color:"#fff",fontSize:14,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center"}} onClick={()=>toast.info(""+dl.vendor.name)}><Icon name="phone" size={18}/></button>
          </div>
        </>:<>
          <div style={{fontSize:15,fontWeight:700,marginBottom:2}}>{dl.client.name}{dl.isGroup&&<span style={{marginLeft:6,padding:"2px 8px",borderRadius:5,background:"rgba(59,130,246,0.08)",color:"#3B82F6",fontSize:10,fontWeight:700}}><Icon name="handshake" size={16}/>{" "}{dl.groupMembers?.length||0} pers.</span>}</div>
          <div style={{fontSize:13,color:"var(--muted)"}}>{dl.client.addr}</div>
          <div style={{display:"flex",gap:8,marginTop:10}}>
            <button style={{flex:1,padding:10,borderRadius:10,border:"none",background:"#F59E0B",color:"#fff",fontSize:12,fontWeight:600,cursor:"pointer",fontFamily:"inherit"}} onClick={()=>go("drChatClient",dl)}><Icon name="chat" size={16}/>{" "}Client</button>
            <button style={{width:42,padding:10,borderRadius:10,border:"none",background:"#3B82F6",color:"#fff",fontSize:14,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center"}} onClick={()=>go("drNavigation",dl)}><Icon name="map" size={18}/></button>
            <button style={{width:42,padding:10,borderRadius:10,border:"none",background:"#F59E0B",color:"#fff",fontSize:14,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center"}} onClick={()=>toast.info(""+dl.client.name)}><Icon name="phone" size={18}/></button>
          </div>
        </>}
      </div>
      ...
      {step>=2&&<div style={{padding:14,background:"rgba(249,115,22,0.04)",border:"1px solid rgba(249,115,22,0.1)",borderRadius:14,marginBottom:14,display:"flex",alignItems:"center",gap:12}}>
        ...
        <button style={{padding:"8px 14px",borderRadius:10,border:"none",background:"#F97316",color:"#fff",fontSize:11,fontWeight:600,cursor:"pointer",fontFamily:"inherit"}} onClick={()=>go("drChatVendor",dl)}><Icon name="chat" size={18}/></button>
        <button style={{padding:"8px 10px",borderRadius:10,border:"1px solid var(--border)",background:"var(--card)",fontSize:14,cursor:"pointer"}} onClick={()=>toast.info(""+dl.vendor.name)}><Icon name="phone" size={18}/></button>
      </div>}
      ...
      <div style={{paddingTop:20,paddingBottom:16}}>
        ...
        :<><button style={{width:"100%",padding:14,borderRadius:14,border:"none",background:"#F59E0B",color:"#fff",fontSize:15,fontWeight:700,cursor:"pointer",fontFamily:"inherit"}} onClick={()=>go("drConfirm",dl)}><Icon name="check_circle" size={16}/>{" "} Checklist & Confirmer</button>
          <button onClick={()=>setShowFail(true)} style={{width:"100%",padding:12,borderRadius:14,border:"1px solid rgba(239,68,68,0.3)",background:"transparent",color:"#EF4444",fontSize:13,fontWeight:600,cursor:"pointer",fontFamily:"inherit",marginTop:8}}><Icon name="x_circle" size={16}/>{" "}Problème de livraison</button>
        </>}
      </div>
    </div>
    ...
    {failDone&&<div style={{position:"fixed",inset:0,background:"var(--bg)",zIndex:200,display:"flex",flexDirection:"column",justifyContent:"center",alignItems:"center",padding:24,animation:"fadeIn .3s ease"}}>
      <div style={{width:80,height:80,borderRadius:"50%",background:"rgba(239,68,68,0.1)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:40,marginBottom:14}}><Icon name="x_circle" size={18}/></div>
    ...
    {showFail&&!failDone&&<div onClick={()=>setShowFail(false)} style={{position:"fixed",inset:0,background:"rgba(0,0,0,.45)",zIndex:9999,display:"flex",alignItems:"flex-end",justifyContent:"center",animation:"fadeInFast .2s ease"}}>
      <div onClick={e=>e.stopPropagation()} style={{width:"100%",maxWidth:500,background:"var(--card)",borderRadius:"20px 20px 0 0",padding:20,maxHeight:"90vh",overflowY:"auto",animation:"slideUp .3s cubic-bezier(.4,0,.2,1)"}}>
        <div style={{textAlign:"center",marginBottom:14}}>
          <div style={{fontSize:32,marginBottom:6}}><Icon name="x_circle" size={18}/></div>
        ...
        <div style={{fontSize:13,fontWeight:700,marginTop:12,marginBottom:6}}><Icon name="camera" size={16}/>{" "}Photo preuve <span style={{color:"#EF4444"}}>*</span></div>
        ...
        {!failPhoto?<div onClick={()=>document.getElementById("fail-photo")?.click()} style={{height:100,background:"var(--light)",border:"2px dashed var(--border)",borderRadius:14,display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer",marginBottom:10}}>
          <div style={{textAlign:"center",color:"var(--muted)",fontSize:12}}><Icon name="camera" size={16}/>{" "}Prendre une photo</div>
        </div>
