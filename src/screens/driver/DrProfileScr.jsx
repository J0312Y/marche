...
function DrProfileScr({go,onSwitch,onLogout}){
  return(<PullToRefresh onRefresh={async()=>{toast.success("Profil actualisé 👤")}}><div className="scr">
    ...
    <div className="vendor-cta" style={{background:"var(--card)",border:"1px solid var(--border)",borderLeft:"4px solid #F97316",color:"var(--text)"}} onClick={onSwitch}><span style={{fontSize:28}}>🛍️</span><div style={{flex:1}}><div style={{fontSize:15,fontWeight:700}}>Mode Acheteur</div><div style={{fontSize:12,opacity:.8,marginTop:2}}>Retourner au marketplace</div></div><span style={{fontSize:18}}>→</span></div>
