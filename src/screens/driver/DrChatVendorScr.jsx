...
function DrChatVendorScr({delivery:dl,onBack}){
  ...
  return(<div style={{display:"flex",flexDirection:"column",height:"100%"}}>
    <div className="chat-head"><button onClick={onBack} style={{width:36,height:36,borderRadius:10,border:"1px solid var(--border)",background:"var(--card)",cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center"}}><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><path d="M19 12H5M12 19l-7-7 7-7"/></svg></button><div style={{position:"relative"}}><div className="ch-av" style={{background:"linear-gradient(135deg,#F97316,#FB923C)",overflow:"hidden"}}>{dl.vendor.logo?<img src={dl.vendor.logo} style={{width:"100%",height:"100%",objectFit:"cover"}} alt=""/>:dl.vendor.avatar}</div><div style={{position:"absolute",bottom:-1,right:-1,width:10,height:10,borderRadius:"50%",background:"#F97316",border:"2px solid #fff"}}/></div><div className="ch-info"><h4>{dl.vendor.name}</h4><p style={{fontSize:11,color:"var(--muted)"}}><Icon name="check_circle" size={16}/>{" "}En ligne ·  Bacongo · {dl.ref}</p></div><button className="ch-call" onClick={()=>toast.info("Appel")}><Icon name="phone" size={18}/></button></div>
