...
function DrChatClientScr({delivery:dl,onBack}){
  ...
  useEffect(()=>{setTimeout(()=>{setMsgs([{from:"bot",text:`Bonjour ${dl.client.name} ! Je suis votre livreur, j'ai récupéré votre commande ${dl.ref} \nJe suis en route !`,time:now(),isWelcome:true}])},500);return()=>{}},[]);
  useEffect(()=>{ref.current&&(ref.current.scrollTop=ref.current.scrollHeight)},[msgs,typing]);

  const quickReplies=[["","Je suis à 500m"],["","Au portail"],["","Je vous appelle"],["⏱️","2 minutes"]];
  const botReplies=["D'accord, j'attends !","Merci, je suis prêt(e) !","OK parfait !","Je suis au portail","Prenez votre temps !"];
  ...
  return(<div style={{display:"flex",flexDirection:"column",height:"100%"}}>
    <div className="chat-head"><button onClick={onBack} style={{width:36,height:36,borderRadius:10,border:"1px solid var(--border)",background:"var(--card)",cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center"}}><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><path d="M19 12H5M12 19l-7-7 7-7"/></svg></button><div style={{position:"relative"}}><div className="ch-av" style={{overflow:"hidden",padding:0}}><img src={CHAT_AVATARS.client} style={{width:"100%",height:"100%",objectFit:"cover"}} alt=""/></div><div style={{position:"absolute",bottom:-1,right:-1,width:10,height:10,borderRadius:"50%",background:"#F97316",border:"2px solid #fff"}}/></div><div className="ch-info"><h4>{dl.client.name}</h4><p style={{fontSize:11,color:"var(--muted)"}}><Icon name="check_circle" size={16}/>{" "}En ligne ·  {dl.client.addr.split(",")[0]} · {dl.ref}</p></div><button className="ch-call" onClick={()=>toast.info(""+dl.client.phone)}><Icon name="phone" size={18}/></button></div>
    <div className="chat-body" ref={ref}>
      {msgs.map((m,i)=><div key={i} className={`msg ${m.from==="user"?"user":"bot"}`} style={m.from==="user"?{background:"#F97316"}:{}}>
        {m.isWelcome&&<div style={{width:24,height:24,borderRadius:6,overflow:"hidden",marginBottom:4}}><img src={CHAT_AVATARS.client} style={{width:"100%",height:"100%",objectFit:"cover"}} alt=""/></div>}
        ...
        {m.text&&m.text.split("\n").map((line,j)=><span key={j}>{line}{j<m.text.split("\n").length-1&&<br/>}</span>)}<div className="msg-time">{m.time}</div>
      </div>)}
    ...
    <div style={{padding:"8px 16px",background:"var(--light)",borderTop:"1px solid var(--border)",display:"flex",gap:6,flexShrink:0,overflowX:"auto"}}>{quickReplies.map(([ic,msg])=><button key={ic} onClick={()=>sendMsg(`${ic} ${msg}`)} style={{padding:"6px 12px",borderRadius:20,border:"1px solid #F97316",background:"rgba(249,115,22,0.04)",color:"#F97316",fontSize:11,fontWeight:600,cursor:"pointer",fontFamily:"inherit",whiteSpace:"nowrap",flexShrink:0}}>{ic} {msg}</button>)}</div>
    <div className="chat-input"><button className="chat-attach" onClick={()=>fileRef.current?.click()}></button><input ref={fileRef} type="file" accept="image/*,.pdf,.doc,.docx" style={{display:"none"}} onChange={handleFileUpload}/><input placeholder="Message au client..." value={inp} onChange={e=>setInp(e.target.value)} onKeyDown={e=>e.key==="Enter"&&sendMsg()}/><button onClick={()=>sendMsg()} style={{background:"#F97316"}}></button></div>
