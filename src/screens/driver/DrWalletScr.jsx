...
function DrWalletScr({go,onBack}){
  ...
  return(<div className="scr"><div className="appbar"><button onClick={onBack}>←</button><h2>Mes gains</h2><div style={{width:38}}/></div>
    ...
    <div style={{padding:"0 16px"}}><div style={{padding:16,background:"var(--card)",border:"1px solid var(--border)",borderRadius:16,marginBottom:14}}>
      ...
      {[["Courses","64 × 2 500 FCFA",fmt(160000)],["Pourboires","23 reçus",fmt(12000)],["Bonus","10+ livrais./sem.",fmt(5000)],["Commissions Lamuka","-5%","-"+fmt(8850)],["Net versé","",""]].map(([l,d,v],i)=><div key={l} style={{display:"flex",justifyContent:"space-between",padding:"8px 0",borderTop:i?"1px solid var(--border)":"none",fontSize:13,...(i===4?{fontWeight:700,fontSize:15}:{})}}><div><span>{l}</span>{d&&<span style={{display:"block",fontSize:11,color:"var(--muted)"}}>{d}</span>}</div><b style={{color:i===3?"#EF4444":i===4?"#10B981":"var(--text)"}}>{i===4?fmt(balance):v}</b></div>)}
    </div></div>
