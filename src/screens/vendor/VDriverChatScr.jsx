...
function VDriverChatScr({delivery:d,onBack}){
  ...
  useEffect(()=>{setTimeout(()=>{setMsgs([{from:"bot",text:`Bonjour ! C'est ${driverName}, votre livreur. Je suis en route pour récupérer la commande `,time:now(),isWelcome:true}])},500);return()=>{}},[]);
  useEffect(()=>{ref.current&&(ref.current.scrollTop=ref.current.scrollHeight)},[msgs,typing]);

  const quickReplies=["La commande est prête","Où êtes-vous ?","Attention fragile","Merci !"];
  const botReplies=["J'arrive dans 5 min !","Compris, je fais attention.","D'accord noté !","Pas de souci.","Je suis devant le magasin !"];
  ...
  return(<div style={{display:"flex",flexDirection:"column",height:"100%"}}>
    <div className="chat-head"><button onClick={onBack} style={{width:36,height:36,borderRadius:10,border:"1px solid var(--border)",background:"var(--card)",cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center"}}><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><path d="M19 12H5M12 19l-7-7 7-7"/></svg></button><div style={{position:"relative"}}><div className="ch-av" style={{overflow:"hidden",padding:0}}><img src={DRIVER_PHOTO} style={{width:"100%",height:"100%",objectFit:"cover"}} alt=""/></div><div style={{position:"absolute",bottom:-1,right:-1,width:10,height:10,borderRadius:"50%",background:"#10B981",border:"2px solid #fff"}}/></div><div className="ch-info"><h4>{driverName}</h4><p style={{fontSize:11,color:"var(--muted)"}}><Icon name="check_circle" size={16}/>{" "}En ligne ·  Talangaï ·  Livreur</p></div><button className="ch-call" onClick={()=>toast.info("Appel")}><Icon name="phone" size={18}/></button></div>
    <div className="chat-body" ref={ref}>
      {msgs.map((m,i)=><div key={i} className={`msg ${m.from==="user"?"user":"bot"}`}>
        {m.isWelcome&&<div style={{width:24,height:24,borderRadius:6,overflow:"hidden",marginBottom:4}}><img src={DRIVER_PHOTO} style={{width:"100%",height:"100%",objectFit:"cover"}} alt=""/></div>}
      ...
      {msgs.length===1&&msgs[0].isWelcome&&<div style={{display:"flex",gap:6,flexWrap:"wrap",padding:"8px 0"}}>{quickReplies.map(q=><button key={q} onClick={()=>sendMsg(q)} style={{padding:"8px 14px",borderRadius:20,border:"1px solid #F97316",background:"rgba(249,115,22,0.04)",color:"#F97316",fontSize:12,fontWeight:600,cursor:"pointer",fontFamily:"inherit"}}>{q}</button>)}</div>}
    </div>
    <div className="chat-input"><button className="chat-attach" onClick={()=>fileRef.current?.click()}></button><input ref={fileRef} type="file" accept="image/*,.pdf,.doc,.docx" style={{display:"none"}} onChange={handleFileUpload}/><input placeholder="Message au livreur..." value={inp} onChange={e=>setInp(e.target.value)} onKeyDown={e=>e.key==="Enter"&&sendMsg()}/><button onClick={()=>sendMsg()}></button></div>
