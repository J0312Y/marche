...
const SUGGESTIONS=[
  {icon:"tag",title:"Lancez une promo -15%",desc:"Vos concurrents ont des promos actives. Une remise de 15% pourrait augmenter vos ventes de 40%."},
  {icon:"camera",title:"Améliorez vos photos",desc:"Les produits avec 3+ photos se vendent 2x plus. 4 de vos produits n'ont qu'une photo."},
  {icon:"⏰",title:"Publiez entre 18h-20h",desc:"C'est le pic de trafic sur Lamuka. Vos stories à cette heure auront 3x plus de vues."},
  {icon:"fire",title:"Ajoutez des ventes flash",desc:"Les vendeurs avec ventes flash ont +60% de commandes. Essayez sur vos 3 meilleurs produits."},
];
...
function VAdvisorScr({onBack,vendorPlan}){
  ...
  return(<div className="scr" style={{padding:16,paddingBottom:20}}>
    ...
    {tab===0&&<>
      <div style={{fontSize:14,fontWeight:700,marginBottom:10}}><Icon name="info" size={16}/>{" "}Recommandations personnalisées</div>
      {SUGGESTIONS.map((s,i)=>(
        <div key={i} style={{padding:14,background:"var(--card)",border:"1px solid var(--border)",borderRadius:14,marginBottom:8,display:"flex",gap:12}}>
          <span style={{fontSize:24}}><Icon name={s.icon} size={20}/></span>
    ...
    {tab===1&&<>
      ...
      {!isEnterprise&&<div style={{marginTop:12,padding:12,background:"rgba(245,158,11,0.06)",borderRadius:12,fontSize:11,color:"var(--muted)",textAlign:"center"}}><Icon name="shield" size={16}/>{" "}Données détaillées par zone disponibles avec le plan Enterprise</div>}
    </>}

    {tab===2&&<>
      <div style={{fontSize:14,fontWeight:700,marginBottom:10}}><Icon name="store" size={16}/>{" "}Benchmark concurrents</div>
      {isEnterprise?COMPETITORS.map(c=>(
        ...
        <div style={{fontSize:36,marginBottom:8}}><Icon name="shield" size={18}/></div>
