...
function VProfileScr({go,onSwitch,vendorPlan,onLogout}){
  ...
  return(<PullToRefresh onRefresh={async()=>{toast.success("Espace actualisé 🏪")}}><div className="scr" style={{paddingBottom:20}}>
    ...
    <div className="vs-header">
      ...
      <div style={{display:"inline-flex",alignItems:"center",gap:6,padding:"4px 12px",borderRadius:20,background:vendorPlan==="enterprise"?"rgba(245,158,11,0.08)":vendorPlan==="pro"?"rgba(249,115,22,0.08)":"var(--light)",marginTop:6}}>
    ...
    <div style={{margin:"0 16px 12px",padding:12,background:"var(--card)",border:"1px solid var(--border)",borderRadius:14}}>
      <div style={{fontSize:12,fontWeight:700,marginBottom:8,display:"flex",justifyContent:"space-between",alignItems:"center"}}>
        ...
        {vendorPlan!=="enterprise"&&<span style={{fontSize:11,color:"#F97316",cursor:"pointer",fontWeight:600}} onClick={()=>go("vUpgradePlan")}>Voir les plans →</span>}
      </div>
    ...
    {menu.map(([icon,title,subtitle,fn,requiredPlan])=>{
      ...
      return(<div key={title} className="menu-item" onClick={locked?()=>handleLocked([icon,title,subtitle,fn,requiredPlan]):fn} style={locked?{opacity:.6,cursor:"pointer"}:{cursor:"pointer"}}>
        ...
        {locked?<span style={{padding:"3px 10px",borderRadius:8,background:requiredPlan==="enterprise"?"rgba(245,158,11,0.08)":"rgba(249,115,22,0.08)",color:requiredPlan==="enterprise"?"#F59E0B":"#F97316",fontSize:9,fontWeight:700,display:"flex",alignItems:"center",gap:3}}>🔒 {requiredPlan==="enterprise"?"Enterprise":"Pro"}</span>
        :<span className="mi-c">›</span>}
      </div>);
    })}

    {/* Upgrade banner */}
    {vendorPlan==="starter"&&<div style={{margin:"14px 16px",padding:16,background:"linear-gradient(135deg,rgba(249,115,22,0.06),rgba(251,146,60,0.06))",border:"1px solid rgba(249,115,22,0.15)",borderRadius:16,cursor:"pointer"}} onClick={()=>go("vUpgradePlan")}>
      <div style={{display:"flex",alignItems:"center",gap:12}}>
        <div style={{width:44,height:44,borderRadius:14,background:"linear-gradient(135deg,#F97316,#FB923C)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:20,color:"#fff",flexShrink:0}}>⬆️</div>
        <div style={{flex:1}}>
          <div style={{fontSize:14,fontWeight:700,color:"#F97316"}}>Passer au plan Pro</div>
          <div style={{fontSize:11,color:"var(--sub)",marginTop:2,lineHeight:1.4}}>Analytics, promotions, rapports, badge vérifié ✓ et plus.</div>
          <div style={{fontSize:12,fontWeight:700,color:"#F97316",marginTop:4}}>15 000 FCFA/mois →</div>
        </div>
      </div>
    </div>}
    ...
    {/* Switch mode */}
    <div className="vendor-cta" style={{background:"linear-gradient(135deg,#3B82F6,#EA580C)"}} onClick={onSwitch}><span style={{fontSize:28}}>🛍️</span><div style={{flex:1}}><div style={{fontSize:15,fontWeight:700}}>Passer en mode Acheteur</div><div style={{fontSize:12,opacity:.8,marginTop:2}}>Retourner au marketplace</div></div><span style={{fontSize:18}}>→</span></div>
