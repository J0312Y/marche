...
function VQAScr({onBack}){
  ...
  return(<div className="scr" style={{padding:16,paddingBottom:20}}>
    <div className="appbar" style={{padding:0,marginBottom:10}}><button onClick={onBack}>←</button><h2><Icon name="help" size={16}/>{" "}Questions ({pending.length} en attente)</h2><div style={{width:38}}/></div>
    ...
    {shown.length===0&&<div style={{textAlign:"center",padding:"40px 0"}}><div style={{fontSize:36}}><Icon name="check_circle" size={18}/></div><div style={{fontSize:13,color:"var(--muted)",marginTop:6}}>{tab==="pending"?"Aucune question en attente":"Aucune réponse"}</div></div>}
    {shown.map(q=><div key={q.id} style={{padding:14,background:"var(--card)",border:replyTo===q.id?"2px solid #F97316":"1px solid var(--border)",borderRadius:16,marginBottom:10}}>
      <div style={{fontSize:10,color:"var(--muted)",marginBottom:6}}><Icon name="package" size={16}/>{" "}{q.product} · {q.client} · {q.date}</div>
      <div style={{fontSize:13,fontWeight:600,marginBottom:6}}><Icon name="help" size={16}/>{" "}{q.q}</div>
      {q.answered&&<div style={{padding:10,background:"rgba(249,115,22,0.04)",border:"1px solid rgba(249,115,22,0.1)",borderRadius:12,fontSize:12,color:"var(--sub)"}}><span style={{color:"#F97316",fontWeight:600}}>↩️ Vous :</span> {q.a}</div>}
      {!q.answered&&replyTo!==q.id&&<button onClick={()=>{setReplyTo(q.id);setReplyText("")}} style={{padding:"7px 14px",borderRadius:10,border:"1px solid #F97316",background:"transparent",color:"#F97316",fontSize:11,fontWeight:600,cursor:"pointer",fontFamily:"inherit"}}>↩️ Répondre</button>}
