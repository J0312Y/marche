  ...
  return(<div className="scr" style={{padding:16,paddingBottom:80}}>
    ...
    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:8,marginBottom:14}}>
      {[["👁️",site.analytics.visitors,"Visiteurs"],["📦",site.analytics.orders,"Commandes"],["📈",site.analytics.conversion,"Conversion"]].map(([icon,val,label])=>(
        <div key={label} style={{padding:12,background:"var(--card)",border:"1px solid var(--border)",borderRadius:14,textAlign:"center"}}>
    ...
    <div style={{marginBottom:14}}>
      ...
      {showPreview&&<div style={{border:"1px solid var(--border)",borderRadius:14,overflow:"hidden",background:"var(--card)"}}>
        {/* Mini browser bar */}
        <div style={{padding:"6px 10px",background:"var(--light)",display:"flex",alignItems:"center",gap:6}}>
          ...
          <div style={{flex:1,padding:"3px 8px",borderRadius:6,background:"var(--card)",fontSize:9,color:"var(--muted)"}}>{site.subdomain}.lamuka.market</div>
        </div>
        {/* Site preview */}
        <div style={{padding:0}}>
          ...
          <div style={{padding:10}}>
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:4}}>
              {[1,2,3].map(i=><div key={i} style={{height:40,borderRadius:6,background:"var(--light)"}}/>)}
            </div>
            <div style={{marginTop:6,height:6,width:"60%",background:"var(--light)",borderRadius:3}}/>
            <div style={{marginTop:3,height:6,width:"40%",background:"var(--light)",borderRadius:3}}/>
          </div>
        </div>
      </div>}
    </div>
    ...
    {!editing?(
      ...
      <div>
        ...
        <div className="field"><label>Description</label><textarea value={site.description} onChange={e=>setSite({...site,description:e.target.value})} rows={2} style={{width:"100%",padding:10,borderRadius:12,border:"1px solid var(--border)",fontSize:12,fontFamily:"inherit",outline:"none",boxSizing:"border-box",resize:"vertical"}}/></div>
        <div className="field"><label>Sous-domaine</label>
          <div style={{display:"flex",alignItems:"center",gap:0}}>
            ...
            <span style={{padding:"12px 10px",background:"var(--light)",border:"1px solid var(--border)",borderRadius:"0 12px 12px 0",fontSize:11,color:"var(--muted)",whiteSpace:"nowrap"}}>.lamuka.market</span>
          </div>
        </div>
        ...
        <div style={{marginBottom:12}}>
          <label style={{fontSize:12,fontWeight:600,color:"var(--sub)",display:"block",marginBottom:6}}>Couleur principale</label>
        ...
        <div style={{display:"flex",gap:10,marginTop:16}}>
          <button onClick={()=>setEditing(false)} style={{flex:1,padding:12,borderRadius:12,border:"1px solid var(--border)",background:"var(--card)",fontSize:13,fontWeight:600,cursor:"pointer",fontFamily:"inherit"}}>Annuler</button>
    ...
    <div style={{marginTop:14,padding:14,background:"rgba(245,158,11,0.04)",border:"1px solid rgba(245,158,11,0.15)",borderRadius:14,display:"flex",alignItems:"center",gap:10}}>
      ...
      <div style={{flex:1}}>
        ...
        <div style={{fontSize:11,color:"var(--muted)"}}>Votre site web est inclus dans votre abonnement Enterprise.</div>
      </div>
    </div>
  </div>);
}
