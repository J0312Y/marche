...
function VLiveScr({onBack}){
  ...
  if(step==="setup")return(<div className="scr" style={{padding:16,paddingBottom:20}}>
    ...
    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10}}>
      <div style={{fontSize:14,fontWeight:700}}><Icon name="package" size={16}/>{" "}Produits à présenter</div>
    ...
    <div style={{padding:12,background:"rgba(59,130,246,0.04)",border:"1px solid rgba(59,130,246,0.1)",borderRadius:12,marginBottom:16}}>
      <div style={{fontSize:12,fontWeight:700,color:"#3B82F6",marginBottom:6}}><Icon name="info" size={16}/>{" "}Conseils pour un bon live</div>
  ...
  if(step==="ended")return(<div className="scr" style={{padding:16}}>
    ...
    {soldItems.length>0&&<div style={{padding:14,background:"rgba(16,185,129,0.04)",border:"1px solid rgba(16,185,129,0.15)",borderRadius:14,marginBottom:14}}>
      <div style={{fontSize:13,fontWeight:700,color:"#10B981",marginBottom:8}}><Icon name="cart" size={16}/>{" "}Produits vendus</div>
    ...
    <div style={{padding:14,background:"var(--card)",border:"1px solid var(--border)",borderRadius:14,marginBottom:14}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10}}>
        <span style={{fontSize:13,fontWeight:700}}><Icon name="chat" size={16}/>{" "}Déroulé du live ({comments.length})</span>
      </div>
    ...
    {/* Products presented */}
    <div style={{padding:14,background:"var(--card)",border:"1px solid var(--border)",borderRadius:14,marginBottom:14}}>
      <div style={{fontSize:13,fontWeight:700,marginBottom:8}}><Icon name="package" size={16}/>{" "}Produits présentés ({selectedProducts.length})</div>
      {selectedProducts.map(id=>{const p=P.find(x=>x.id===id);const sold=soldItems.includes(id);return p?<div key={id} style={{display:"flex",alignItems:"center",gap:8,padding:"6px 0"}}>
        ...
        {sold?<span style={{padding:"2px 6px",borderRadius:4,background:"rgba(16,185,129,0.08)",color:"#10B981",fontSize:9,fontWeight:700}}><Icon name="check_circle" size={16}/>{" "}Vendu</span>
  ...
  return(<div style={{display:"flex",flexDirection:"column",height:"100%",background:"#000"}}>
    ...
    <div style={{flex:1,position:"relative",background:"linear-gradient(135deg,#1a1a2e,#16213e)",minHeight:300}}>
      <div style={{position:"absolute",inset:0,display:"flex",alignItems:"center",justifyContent:"center"}}><span style={{fontSize:80,opacity:.15}}><Icon name="camera" size={18}/></span></div>
      ...
      <div style={{position:"absolute",top:12,left:12,right:12,display:"flex",justifyContent:"space-between",alignItems:"center",zIndex:10}}>
        <div style={{display:"flex",gap:6}}>
          <div style={{padding:"4px 10px",borderRadius:20,background:"#EF4444",color:"#fff",fontSize:10,fontWeight:700,display:"flex",alignItems:"center",gap:4,animation:"pulse 1.5s infinite"}}><Icon name="x_circle" size={16}/>{" "}LIVE</div>
          <div style={{padding:"4px 10px",borderRadius:20,background:"rgba(0,0,0,.5)",color:"#fff",fontSize:10,fontWeight:600}}><Icon name="map" size={16}/>{" "}{viewers}</div>
    ...
    <div style={{background:"var(--card)",padding:10,borderTop:"1px solid var(--border)"}}>
      ...
      <div style={{display:"flex",gap:6,overflowX:"auto",scrollbarWidth:"none",marginBottom:8,paddingBottom:4}}>
        {selectedProducts.map(id=>{const p=P.find(x=>x.id===id);return p?<div key={id} onClick={()=>setPinnedProduct(pinnedProduct===id?null:id)} style={{display:"flex",alignItems:"center",gap:6,padding:"6px 10px",background:pinnedProduct===id?"rgba(249,115,22,0.08)":"var(--light)",border:pinnedProduct===id?"1px solid #F97316":"1px solid var(--border)",borderRadius:10,flexShrink:0,cursor:"pointer"}}>
          ...
          {soldItems.includes(id)&&<span style={{fontSize:10}}><Icon name="check_circle" size={18}/></span>}
        </div>:null})}
      </div>

      {/* Action buttons */}
      <div style={{display:"flex",gap:6}}>
        <button onClick={()=>setShowPromo(!showPromo)} style={{padding:"8px 12px",borderRadius:10,border:"1px solid var(--border)",background:showPromo?"rgba(249,115,22,0.06)":"var(--card)",fontSize:11,fontWeight:600,cursor:"pointer",fontFamily:"inherit",color:"var(--text)"}}><Icon name="tag" size={16}/>{" "}Promo</button>
      ...
      {showPromo&&<div style={{marginTop:8,padding:10,background:"rgba(239,68,68,0.04)",border:"1px solid rgba(239,68,68,0.15)",borderRadius:10}}>
        <div style={{fontSize:12,fontWeight:700,color:"#EF4444",marginBottom:6}}><Icon name="tag" size={16}/>{" "}Lancer une promo flash</div>
        <div style={{display:"flex",gap:6}}>
