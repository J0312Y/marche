...
function VAssignDriverScr({order:o,onBack,go}){
  ...
  if(step===2)return(<div style={{display:"flex",flexDirection:"column",height:"100%",justifyContent:"center"}}>
    <div style={{textAlign:"center",padding:"40px 24px"}}>
      <div style={{width:80,height:80,borderRadius:"50%",background:"rgba(16,185,129,0.1)",display:"flex",alignItems:"center",justifyContent:"center",margin:"0 auto 16px",fontSize:40,animation:"splash-pop .5s ease"}}><Icon name="check_circle" size={18}/></div>
      ...
      <div style={{display:"flex",gap:10}}>
        <button style={{flex:1,padding:14,borderRadius:14,border:"none",background:"#F97316",color:"#fff",fontSize:14,fontWeight:600,cursor:"pointer",fontFamily:"inherit"}} onClick={()=>go("vDriverChat",{ref:o.ref,driver:chosen.name,driverAv:chosen.avatar})}><Icon name="chat" size={16}/>{" "}Contacter le livreur</button>
      </div>
  ...
  return(<>
    ...
    <div className="scr" style={{padding:16}}>
      ...
      <div style={{padding:14,background:"var(--card)",border:"1px solid var(--border)",borderRadius:16,marginBottom:12,display:"flex",alignItems:"center",gap:12}}>
        <div style={{width:44,height:44,borderRadius:12,background:"rgba(249,115,22,0.1)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:20,flexShrink:0}}><Icon name="package" size={18}/></div>
      ...
      {step===0&&<>
        ...
        {drivers.filter(d=>d.status==="busy").length>0&&<div className="info-box yellow" style={{marginTop:6}}><span><Icon name="info" size={18}/></span><span>Les livreurs occupés seront disponibles une fois leur livraison en cours terminée.</span></div>}
      </>}
      ...
      {step===1&&<>
        ...
        <div style={{padding:16,background:"var(--card)",border:"1px solid var(--border)",borderRadius:16,marginBottom:14}}>
          <div style={{display:"flex",alignItems:"center",gap:12,marginBottom:14}}>
            ...
            <div><div style={{fontSize:16,fontWeight:700}}>{chosen?.name}</div><div style={{fontSize:13,color:"var(--muted)"}}>{chosen?.vehicle} · {chosen?.plate}</div><div style={{fontSize:12,color:"#F59E0B",marginTop:2}}><Icon name="star_full" size={16}/>{" "}{chosen?.rating} · {chosen?.deliveries} livraisons</div></div>
          </div>
        ...
        <div className="info-box green"><span><Icon name="phone" size={18}/></span><span>Le livreur recevra une notification avec les détails de la commande et l'adresse du client.</span></div>
      </>}
