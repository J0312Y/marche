...
function VAddShopScr({onBack}){
  ...
  const shopTypes=[
    {id:"boutique",icon:"store",name:"Boutique",desc:"Mode, électronique, artisanat, accessoires..."},
    {id:"restaurant",icon:"utensils",name:"Restaurant",desc:"Plats cuisinés, snacks, boissons, traiteur"},
    {id:"patisserie",icon:"cupcake",name:"Pâtisserie / Boulangerie",desc:"Gâteaux, pains, viennoiseries, confiseries"},
    {id:"supermarche",icon:"cart",name:"Supermarché / Épicerie",desc:"Produits alimentaires, ménagers, du quotidien"},
    {id:"pharmacie",icon:"pill",name:"Pharmacie / Parapharmacie",desc:"Médicaments, cosmétiques, produits de santé"},
    {id:"service",icon:"tool",name:"Service",desc:"Réparation, beauté, impression, pressing..."},
  ];
  ...
  return(<>
    ...
    <div className="scr" style={{padding:16}}>
      ...
      {step===0&&<>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
          {shopTypes.map(t=><div key={t.id} onClick={()=>setShopType(t.id)} style={{padding:16,background:shopType===t.id?"rgba(249,115,22,0.06)":"var(--card)",border:shopType===t.id?"2px solid #F97316":"2px solid var(--border)",borderRadius:16,cursor:"pointer",textAlign:"center",transition:"all .15s",position:"relative"}}>
            ...
            <div style={{fontSize:32,marginBottom:6}}><Icon name={t.icon} size={20}/></div>
      ...
      {step===1&&<>
        ...
        <div className="field"><label>Téléphone <span style={{color:"#EF4444"}}>*</span></label><input placeholder="04/05/06X XXX XXX" maxLength={11} type="tel"/></div>
      ...
      {step===3&&<>
        ...
        <div className="info-box green"><span><Icon name="check_circle" size={18}/></span><span style={{fontSize:11}}>Ce{shopType==="restaurant"?" restaurant":shopType==="patisserie"?"tte pâtisserie":"tte boutique"} bénéficiera de votre plan Enterprise : 2% commission, dashboard personnalisé, manager dédié.</span></div>
      </>}
