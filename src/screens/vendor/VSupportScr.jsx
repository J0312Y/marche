...
function VSupportScr({go,onBack,vendorPlan}){
  ...
  const faqCats=[
    {cat:"Commandes",icon:"package",items:[
    ...
    {cat:"Articles",icon:"package",items:[
    ...
    {cat:"Paiements",icon:"creditCard",items:[
    ...
    {cat:"Livraison",icon:"truck",items:[
    ...
    ...(isPro?[{cat:"Promotions",icon:"tag",items:[
  ...
  const guides=[
    {id:"onboarding",icon:"rocket",title:"Guide de démarrage",desc:"Premiers pas, configuration commerce, premier article",sections:[
    ...
    {id:"orders",icon:"package",title:"Gestion des commandes",desc:"Cycle de vie, traitement, assignation livreur",sections:[
    ...
    {id:"payments",icon:"wallet",title:"Paiements & Facturation",desc:"Commissions, retraits, comptabilité",sections:[
    ...
    ...(isEnt?[
      ...
      {id:"sla",icon:"document",title:"SLA & Engagement de service",desc:"Garanties, temps de réponse, disponibilité",sections:[
  ...
  return(<div className="scr">
    ...
    {isEnt&&<div style={{margin:"0 20px 14px",padding:14,background:"linear-gradient(135deg,#F59E0B,#D97706)",borderRadius:14,color:"#fff"}}>
      <div style={{display:"flex",alignItems:"center",gap:10}}>
        <span style={{fontSize:24}}><Icon name="star_full" size={18}/></span>
        <div>
    ...
    {search&&<div style={{padding:"0 20px 14px"}}>
      ...
      {filtered.map((f,i)=><div key={i} style={{padding:12,background:"var(--card)",border:"1px solid var(--border)",borderRadius:12,marginBottom:8,cursor:"pointer"}} onClick={()=>{setOpen(open===`s${i}`?null:`s${i}`)}}>
        <div style={{display:"flex",alignItems:"center",gap:6,marginBottom:4}}><span style={{fontSize:12}}><Icon name={f.icon} size={20}/></span><span style={{fontSize:10,color:"var(--muted)"}}>{f.cat}</span></div>
    ...
    {!search&&<>
      ...
      {tab===2&&<div style={{padding:"0 16px 20px"}}>
        {isEnt&&<div style={{padding:16,background:"var(--card)",borderRadius:16,marginBottom:14,color:"var(--text)",border:"1px solid var(--border)"}}>
          <h4 style={{fontSize:15,fontWeight:700,marginBottom:6}}><Icon name="user" size={16}/>{" "}Votre manager dédié</h4>
          <div style={{fontSize:13,fontWeight:600,marginBottom:4}}>Sophie Mabika</div>
          <div style={{fontSize:11,color:"var(--muted)",lineHeight:1.8}}><Icon name="mail" size={16}/>{" "}sophie.mabika@lamuka.cg{"\n"} +242 06X XXX XXX{"\n"} WhatsApp disponible{"\n"}⏱️ Lun-Ven 8h-18h · Réponse sous 2h</div>
          <div style={{display:"flex",gap:8,marginTop:10}}>
            <button onClick={()=>window.location.href="tel:+242064663469"} style={{flex:1,padding:10,borderRadius:10,border:"none",background:"#F97316",color:"#fff",fontSize:12,fontWeight:700,cursor:"pointer",fontFamily:"inherit"}}><Icon name="phone" size={16}/>{" "}Appeler</button>
            <button onClick={()=>window.open("https://wa.me/242064663469?text=Bonjour, je suis vendeur sur Lamuka Market et j'ai besoin d'aide","_blank")} style={{flex:1,padding:10,borderRadius:10,border:"1px solid #F97316",background:"transparent",color:"#F97316",fontSize:12,fontWeight:700,cursor:"pointer",fontFamily:"inherit"}}><Icon name="chat" size={16}/>{" "}WhatsApp</button>
          </div>
        </div>}
