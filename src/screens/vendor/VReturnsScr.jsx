...
function VReturnsScr({onBack}){
  ...
  return(<div className="scr" style={{padding:16,paddingBottom:20}}>
    ...
    {shown.map(r=><div key={r.id} style={{padding:14,background:"var(--card)",border:"1px solid var(--border)",borderRadius:16,marginBottom:10}}>
      ...
      <div style={{fontSize:11,color:"var(--muted)",marginBottom:4}}><Icon name="user" size={16}/>{" "}{r.client} · {r.date} · Commande {r.order}</div>
      <div style={{fontSize:12,color:"var(--sub)",marginBottom:4}}><Icon name="document" size={16}/>{" "}{r.reason}</div>
