...
function VChatScr({chat:c,onBack}){
  ...
  return(<div style={{display:"flex",flexDirection:"column",height:"100%"}}>
    <div className="chat-head"><button onClick={onBack} style={{width:36,height:36,borderRadius:10,border:"1px solid var(--border)",background:"var(--card)",cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center"}}><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><path d="M19 12H5M12 19l-7-7 7-7"/></svg></button><div style={{position:"relative"}}><div className="ch-av" style={{overflow:"hidden",padding:0}}><img src={CHAT_AVATARS.femme1} style={{width:"100%",height:"100%",objectFit:"cover"}} alt=""/></div><div style={{position:"absolute",bottom:-1,right:-1,width:10,height:10,borderRadius:"50%",background:c?.online!==false?"#10B981":"var(--muted)",border:"2px solid #fff"}}/></div><div className="ch-info"><h4>{clientName}</h4><p style={{fontSize:11,color:"var(--muted)"}}>{c?.online!==false?"En ligne":" Hors ligne"} ·  {c?.zone||"Brazzaville"}</p></div><button className="ch-call"><Icon name="phone" size={18}/></button></div>
    <div className="chat-body" ref={ref}>
      {msgs.map((m,i)=><div key={i} className={`msg ${m.from==="user"?"user":"bot"}`}>
        {m.isWelcome&&<div style={{width:24,height:24,borderRadius:6,overflow:"hidden",marginBottom:4}}><img src={CHAT_AVATARS.femme1} style={{width:"100%",height:"100%",objectFit:"cover"}} alt=""/></div>}
