...
function VUpgradePlanScr({onBack,onUpgrade,currentPlan="starter"}){
  ...
  if(done)return(<div className="scr" style={{padding:16,textAlign:"center"}}><div style={{padding:"40px 0"}}><div style={{fontSize:48,marginBottom:10}}>🎉</div><h3 style={{fontSize:18,fontWeight:700}}>Plan mis à jour !</h3><p style={{fontSize:14,fontWeight:700,marginTop:8,color:plan==="enterprise"?"#F59E0B":"#6366F1"}}>Plan {plan==="pro"?"Pro":"Enterprise"} activé</p><p style={{fontSize:13,color:"#908C82",marginTop:6}}>Toutes les fonctionnalités sont maintenant débloquées.</p><button className="btn-primary" style={{marginTop:20}} onClick={()=>{onUpgrade(plan);onBack()}}>✅ Retour à la boutique</button></div></div>);
  return(<><div className="appbar"><button onClick={onBack}>←</button><h2>Changer de plan</h2><div style={{width:38}}/></div>
    <div className="scr" style={{padding:16}}>
      <div style={{textAlign:"center",marginBottom:14}}><div style={{fontSize:48,marginBottom:8}}>⬆️</div><h3 style={{fontSize:18,fontWeight:700}}>Boostez votre commerce</h3><p style={{fontSize:13,color:"#908C82"}}>Choisissez le plan qui correspond à vos ambitions</p></div>
      {currentInfo&&<div style={{padding:16,background:"#F5F4F1",borderRadius:16,marginBottom:14,opacity:.5}}>
        ...
        <p style={{fontSize:12,color:"#908C82",marginTop:4}}>{currentInfo[4].join(" · ")}</p>
      </div>}
      {upgradeable.map(([k,n,pr,tag,f])=><div key={k} onClick={()=>setPlan(k)} style={{padding:16,background:"#fff",border:plan===k?"2px solid "+(k==="enterprise"?"#F59E0B":"#6366F1"):"2px solid #E8E6E1",borderRadius:16,marginBottom:14,cursor:"pointer",position:"relative"}}>
        ...
        <div style={{display:"flex",flexWrap:"wrap",gap:6,marginTop:10}}>{f.map(x=><span key={x} style={{padding:"4px 10px",borderRadius:8,background:k==="enterprise"?"rgba(245,158,11,0.06)":"rgba(99,102,241,0.06)",color:k==="enterprise"?"#F59E0B":"#6366F1",fontSize:10,fontWeight:600}}>✓ {x}</span>)}</div>
      </div>)}
