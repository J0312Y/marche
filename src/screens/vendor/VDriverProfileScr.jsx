...
function VDriverProfileScr({driver:d,go,onBack}){
  ...
  return(<div className="scr" style={{paddingBottom:20}}>
    ...
    <div className="vp-stats" style={{padding:"0 16px",marginBottom:12}}>
      ...
      <div className="vps p"><div className="vsi"><Icon name="package" size={18}/></div><b>{d.deliveries}</b><span>Livraisons</span></div>
    ...
    <div style={{display:"flex",gap:10,padding:"0 16px",marginBottom:12}}>
      <button style={{flex:1,padding:12,borderRadius:14,border:"none",background:"#F59E0B",color:"#fff",fontSize:13,fontWeight:600,cursor:"pointer",fontFamily:"inherit",display:"flex",alignItems:"center",justifyContent:"center",gap:6}} onClick={()=>window.location.href="tel:+242064663469"}><Icon name="phone" size={16}/>{" "}Appeler</button>
      <button style={{flex:1,padding:12,borderRadius:14,border:"none",background:"#F97316",color:"#fff",fontSize:13,fontWeight:600,cursor:"pointer",fontFamily:"inherit",display:"flex",alignItems:"center",justifyContent:"center",gap:6}} onClick={()=>go("vDriverChat",{driver:d.name,driverAv:d.avatar})}><Icon name="chat" size={16}/>{" "}Message</button>
    </div>
    {d.status==="available"&&<div style={{padding:"0 16px",marginBottom:12}}>
      <div className="info-box green"><span><Icon name="check_circle" size={18}/></span><span>{d.name} est disponible pour une livraison. Assignez-le depuis le détail d'une commande.</span></div>
    </div>}
    <div style={{padding:"0 16px"}}><div style={{fontSize:14,fontWeight:700,marginBottom:12}}>Historique récent</div>
      {history.map((h,i)=><div key={i} style={{display:"flex",alignItems:"center",gap:10,padding:"10px 0",borderBottom:"1px solid var(--border)"}}>
        <div style={{width:32,height:32,borderRadius:8,background:"rgba(16,185,129,0.1)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:14}}><Icon name="check_circle" size={18}/></div>
