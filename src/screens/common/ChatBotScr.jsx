...
function ChatBotScr({ onBack, mode = "buyer" }) {
  const MODE_CONTEXT = {
    buyer: "L'utilisateur est un CLIENT qui achète sur Lamuka Market.",
    vendor: "L'utilisateur est un VENDEUR/COMMERÇANT sur Lamuka Market. Il gère sa boutique, ses produits, commandes, livraisons, promos, stats. Plans : Starter (gratuit, 5% commission), Pro (9 900 F/mois, 3%), Enterprise (sur mesure, 2%). Aide-le à optimiser ses ventes.",
    driver: "L'utilisateur est un LIVREUR sur Lamuka Market. Il fait des livraisons à Brazzaville. Aide-le avec : gains, zones, véhicule, navigation, paiements, problèmes de livraison.",
  };
  ...
  return (
    <div className="scr" style={{ display: "flex", flexDirection: "column", height: "100%" }}>
      ...
      <div style={{ padding: "6px 12px", display: "flex", gap: 6, overflowX: "auto", scrollbarWidth: "none", flexShrink: 0 }}>
        {(mode==="vendor"?QUICK_VENDOR:mode==="driver"?QUICK_DRIVER:QUICK_BUYER).map(([ic, label]) => (
