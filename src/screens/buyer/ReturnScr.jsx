...
function ReturnScr({onBack,order}){
  ...
  if(done)return(<div className="scr" style={{padding:16,textAlign:"center",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",height:"100%"}}><div style={{fontSize:56,marginBottom:14}}><Icon name="check_circle" size={18}/></div><h2 style={{fontSize:20,fontWeight:700,marginBottom:6}}>Demande envoyée</h2><p style={{fontSize:13,color:"var(--muted)",marginBottom:8}}>Nous traiterons votre demande sous 24-48h.</p>
    <div style={{padding:10,background:"rgba(16,185,129,0.06)",borderRadius:10,marginBottom:8,fontSize:12}}>{refundMethod==="wallet"?<span style={{color:"#10B981",fontWeight:600}}><Icon name="wallet" size={16}/>{" "}Remboursement sur votre wallet Lamuka</span>:<span style={{color:"#3B82F6",fontWeight:600}}><Icon name="phone" size={16}/>{" "}Remboursement sur +242 {refundPhone}</span>}</div><p style={{fontSize:12,color:"var(--sub)"}}>Réf: #RET-2026-{Math.floor(Math.random()*9000+1000)}</p><button className="btn-primary" style={{marginTop:20,maxWidth:280}} onClick={onBack}>Retour</button></div>);
  return(<div className="scr" style={{padding:16,paddingBottom:20}}>
    ...
    <div style={{marginBottom:14}}>
      ...
      <div style={{display:"flex",gap:8,flexWrap:"wrap"}}>
        ...
        {photos.length<4&&<div onClick={()=>document.getElementById("ret-photo")?.click()} style={{width:60,height:60,borderRadius:10,border:"2px dashed var(--border)",display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer",fontSize:20}}><Icon name="camera" size={18}/></div>}
