...
function AllProductsScr({go,onBack,favs,toggleFav,isFav}){
  ...
  return(<div className="scr"><div className="appbar"><button onClick={onBack}>←</button><h2>Tous les articles</h2><div style={{width:38}}/></div>
    <div style={{display:"flex",gap:6,padding:"0 20px 10px"}}>{[["popular","🔥 Populaires"],["rating","⭐ Mieux notés"],["price","💰 Prix ↑"]].map(([k,l])=><button key={k} onClick={()=>setSort(k)} style={{padding:"6px 12px",borderRadius:8,border:sort===k?"2px solid #6366F1":"1px solid var(--border)",background:sort===k?"rgba(99,102,241,0.04)":"#fff",fontSize:11,fontWeight:600,cursor:"pointer",fontFamily:"inherit",color:sort===k?"#6366F1":"var(--muted)"}}>{l}</button>)}</div>
