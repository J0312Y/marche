  ...
  return(<div className="scr">
    ...
    <div style={{display:"flex",margin:"0 16px 8px",background:"var(--light)",borderRadius:10,padding:2}}>
      {[["products","🛍️ Produits",fp.length],["vendors","🏪 Boutiques",fv.length]].map(([k,l,c])=>(
        <button key={k} onClick={()=>setTab(k)} style={{flex:1,padding:"7px 0",borderRadius:8,border:"none",background:tab===k?"var(--card)":"transparent",color:tab===k?"#191815":"#908C82",fontSize:11,fontWeight:tab===k?700:500,cursor:"pointer",fontFamily:"inherit",boxShadow:tab===k?"0 1px 3px rgba(0,0,0,.06)":"none"}}>{l} ({c})</button>
      ))}
    </div>
    {tab==="products"&&<>
      {/* Category chips */}
      ...
      <div style={{display:"flex",gap:6,padding:"0 16px 8px"}}>
        {[["popular","🔥 Populaires"],["rating","⭐ Notés"],["priceAsc","💰 Prix ↑"],["priceDesc","💰 Prix ↓"]].map(([k,l])=><button key={k} onClick={()=>setSortBy(k)} style={{padding:"4px 10px",borderRadius:8,border:"none",background:sortBy===k?"#6366F1":"#F5F4F1",color:sortBy===k?"var(--card)":"#908C82",fontSize:9,fontWeight:600,cursor:"pointer",fontFamily:"inherit"}}>{l}</button>)}
      </div>
