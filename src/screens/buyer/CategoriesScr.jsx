  ...
  return (
    <div className="scr">
      ...
      <div style={{ padding: "0 16px 20px", display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
        {CATS.map(c => (
          <div key={c.id} onClick={() => { go("search") }} style={{ background: "var(--card)", border: "1px solid var(--border)", borderRadius: 14, overflow: "hidden", cursor: "pointer", aspectRatio: "1 / 1" }}>
            <div style={{ width: "100%", height: "70%", background: "var(--light)", position: "relative", overflow: "hidden" }}>
              {c.photo ? <img src={c.photo} alt={c.name} style={{ width: "100%", height: "100%", objectFit: "cover", objectPosition: "center" }} /> : null}
            </div>
