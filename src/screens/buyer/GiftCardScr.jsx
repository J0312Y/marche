...
function GiftCardScr({onBack}){
  ...
  const [amount,setAmount]=useState(null);const [to,setTo]=useState("");
  const [toErr,setToErr]=useState("");const [msg,setMsg]=useState("");const [done,setDone]=useState(false);
  const amounts=[5000,10000,25000,50000];
  const send=()=>{if(!amount||!to){toast.error("Montant et destinataire requis");return}if(to.replace(/\s/g,"").length!==9){toast.error("Le numéro doit contenir 9 chiffres");return}setDone(true);validateGc()&&toast.success("Carte cadeau envoyée ! 🎁")};
  ...
  return(<div className="scr" style={{padding:16,paddingBottom:20}}>
    ...
    <div className="field"><label>Numéro du destinataire <span style={{color:"#EF4444"}}>*</span></label><input value={to} onChange={e=>{const v=e.target.value.replace(/[^0-9]/g,"").slice(0,9);setTo(v);setToErr("")}} placeholder="06X XXX XXX" type="tel" maxLength={11}/></div>
