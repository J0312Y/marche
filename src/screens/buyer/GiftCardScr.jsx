...
function GiftCardScr({onBack,go}){
  ...
  if(done)return(<div className="scr" style={{padding:16,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",height:"100%"}}>
    <div style={{width:240,padding:24,background:"linear-gradient(135deg,#F97316,#FB923C)",borderRadius:20,color:"#fff",textAlign:"center",marginBottom:20,boxShadow:"0 8px 32px rgba(249,115,22,.25)"}}>
      <div style={{fontSize:28,marginBottom:4}}><Icon name="gift" size={18}/></div>
    ...
    <h3 style={{fontSize:18,fontWeight:700,marginBottom:4}}>Carte envoyée ! <Icon name="check_circle" size={16}/></h3>
    ...
    <div style={{display:"flex",gap:8,marginTop:16,width:"100%",maxWidth:280}}>
      <button onClick={()=>copyCode(generatedCode)} style={{flex:1,padding:12,borderRadius:12,border:"1px solid var(--border)",background:"var(--card)",fontSize:12,fontWeight:600,cursor:"pointer",fontFamily:"inherit",color:"var(--text)"}}><Icon name="document" size={16}/>{" "}Copier</button>
    ...
    <button onClick={()=>onBack&&onBack()} style={{marginTop:8,fontSize:12,color:"var(--muted)",background:"none",border:"none",cursor:"pointer",fontFamily:"inherit"}}>Retour au profil</button>
  </div>);

  return(<div className="scr" style={{padding:16,paddingBottom:20}}>
    <div className="appbar" style={{padding:0,marginBottom:10}}><button onClick={()=>onBack&&onBack()}>←</button><h2><Icon name="gift" size={16}/>{" "}Carte Cadeau</h2><div style={{width:38}}/></div>
    ...
    {/* ── Send tab ── */}
    {tab==="send"&&<>
      <div style={{textAlign:"center",marginBottom:14}}><div style={{fontSize:36,marginBottom:6}}><Icon name="gift" size={18}/></div><p style={{fontSize:12,color:"var(--muted)"}}>Offrez du shopping à vos proches</p></div>
    ...
    {tab==="redeem"&&<>
      ...
      {(()=>{
        ...
        return wallet.length>0&&<div style={{marginTop:16}}>
          <div style={{fontSize:13,fontWeight:700,marginBottom:8}}><Icon name="creditCard" size={16}/>{" "}Mes cartes actives</div>
          {wallet.map(w=>(
          ...
          <div style={{fontSize:10,color:"var(--muted)",marginTop:4}}><Icon name="info" size={16}/>{" "}Appliqué automatiquement dans le panier ou utilisez le code en tant que coupon.</div>
        </div>;
      })()}
    </>}
    ...
    {tab==="history"&&<>
      {history.length===0?<div style={{textAlign:"center",padding:"40px 0",color:"var(--muted)"}}>
        <div style={{fontSize:36,marginBottom:8}}><Icon name="document" size={18}/></div>
      ...
      :history.map(card=>(
        <div key={card.id} style={{padding:14,background:"var(--card)",border:"1px solid var(--border)",borderRadius:14,marginBottom:8}}>
          ...
          <div style={{display:"flex",alignItems:"center",gap:8,marginTop:8}}>
            ...
            <button onClick={()=>copyCode(card.code)} style={{padding:"4px 10px",borderRadius:6,border:"1px solid var(--border)",background:"var(--card)",fontSize:10,fontWeight:600,cursor:"pointer",fontFamily:"inherit",color:"var(--text)"}}><Icon name="document" size={16}/>{" "}Copier</button>
          </div>
        </div>
      ))}
    </>}
