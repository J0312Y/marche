...
function CartScr({cart,setCart,go}){
  ...
  return(<><div className="scr" style={{padding:20}}>
    ...
    :cart.map((c,i)=><div key={i} className="cart-item"><div className="cart-img"><Img src={c.product.photo} emoji={c.product.img} style={{width:"100%",height:"100%",borderRadius:12}} fit="cover"/></div><div className="cart-info"><h4>{c.product.name}</h4><div className="cv">{c.product.vendor}</div><div className="cart-bot"><span className="cp">{fmt(c.product.price*c.qty)}</span><div className="qty"><button onClick={()=>updQty(i,-1)}>−</button><span>{c.qty}</span><button onClick={()=>updQty(i,1)}>+</button></div></div></div></div>)}
