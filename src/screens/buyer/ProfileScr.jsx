...
function ProfileScr({go,userRole,vendorPlan,vendorStatus,driverStatus,onLogout}){
  ...
  return(<PullToRefresh onRefresh={async()=>{toast.success("Profil actualisé 👤")}}><div className="scr">
    ...
    {/* Approved vendor */}
    {hasVendor&&<div className="vendor-cta" style={{background:"linear-gradient(135deg,#F97316,#FB923C)"}} onClick={()=>go("switchVendor")}><span style={{fontSize:28}}>🏪</span><div style={{flex:1}}><div style={{fontSize:15,fontWeight:700}}>Espace Commerçant</div><div style={{fontSize:12,opacity:.8,marginTop:2}}>Plan {vendorPlan==="starter"?"Starter":vendorPlan==="pro"?"Pro":"Enterprise"} · Gérer mon commerce</div></div><span style={{fontSize:18}}>→</span></div>}

    {/* Approved driver */}
    {hasDriver&&<div className="vendor-cta" style={{background:"linear-gradient(135deg,#F97316,#EA580C)"}} onClick={()=>go("switchDriver")}><span style={{fontSize:28}}>🛵</span><div style={{flex:1}}><div style={{fontSize:15,fontWeight:700}}>Espace Livreur</div><div style={{fontSize:12,opacity:.8,marginTop:2}}>Gérer mes livraisons et gains</div></div><span style={{fontSize:18}}>→</span></div>}
