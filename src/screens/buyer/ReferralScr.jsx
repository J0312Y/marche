...
function ReferralScr({onBack}){
  ...
  return(<div className="scr" style={{padding:16,paddingBottom:20}}>
    ...
    <div style={{padding:14,background:"var(--card)",border:"1px solid var(--border)",borderRadius:16,marginBottom:14,textAlign:"center"}}>
      ...
      <div style={{fontSize:24,fontWeight:800,letterSpacing:2,color:"#6366F1",marginBottom:10}}>{code}</div>
      <div style={{display:"flex",gap:8}}>
        ...
        <button onClick={shareCode} style={{flex:1,padding:10,borderRadius:12,border:"none",background:"#6366F1",color:"#fff",fontSize:12,fontWeight:600,cursor:"pointer",fontFamily:"inherit"}}>📤 Partager</button>
      </div>
    </div>
    <div style={{display:"flex",gap:8,marginBottom:14}}>
      ...
      <div style={{flex:1,padding:14,background:"var(--card)",border:"1px solid var(--border)",borderRadius:14,textAlign:"center"}}><div style={{fontSize:20,fontWeight:800,color:"#6366F1"}}>{invites.length}</div><div style={{fontSize:10,color:"var(--muted)"}}>Amis invités</div></div>
    </div>
