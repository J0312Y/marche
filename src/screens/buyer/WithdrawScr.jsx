...
function WithdrawScr({onBack,mode}){
  ...
  return(<><div className="appbar"><button onClick={onBack}>←</button><h2>Retrait</h2><div style={{width:38}}/></div>
    <div className="scr" style={{padding:16}}>
      ...
      {[["airtel","📱","Airtel Money","06X XXX XXX"],["mtn","📱","MTN MoMo","06X XXX XXX"],["kolo","🟣","Kolo Pay","06X XXX XXX"]].map(([k,i,n,p])=><div key={k} className="setting-item" style={{cursor:"pointer",border:method===k?`2px solid ${color}`:"1px solid #E8E6E1",marginBottom:6}} onClick={()=>setMethod(k)}><span style={{fontSize:20}}>{i}</span><div style={{flex:1}}><div style={{fontSize:14,fontWeight:500}}>{n}</div><div style={{fontSize:11,color:"var(--muted)"}}>{p}</div></div>{method===k&&<span style={{color,fontWeight:700}}>✓</span>}</div>)}
      <div style={{paddingTop:24,paddingBottom:16}}><button className="btn-primary" style={{background:amount&&method?color:"#E8E6E1",color:amount&&method?"var(--card)":"#908C82"}} onClick={()=>{if(amount&&method){setDone(true);toast.success("Retrait effectué 💸")}}}>Retirer {amount?fmt(parseInt(amount)):""}</button></div>
    </div>
  </>);
}
