...
function WithdrawScr({onBack,mode}){
  ...
  return(<><div className="appbar"><button onClick={onBack}>←</button><h2>Retrait</h2><div style={{width:38}}/></div>
    <div className="scr" style={{padding:16}}>
      ...
      {[["airtel","📱","Airtel Money","04X / 05X XXX XXX"],["mtn","📱","MTN MoMo","06X XXX XXX"],["kolo","🟣","Kolo Pay","04-06X XXX XXX"]].map(([k,i,n,p])=><div key={k} className="setting-item" style={{cursor:"pointer",border:method===k?`2px solid ${color}`:"1px solid var(--border)",marginBottom:6}} onClick={()=>setMethod(k)}><span style={{fontSize:20}}>{i}</span><div style={{flex:1}}><div style={{fontSize:14,fontWeight:500}}>{n}</div><div style={{fontSize:11,color:"var(--muted)"}}>{p}</div></div>{method===k&&<span style={{color,fontWeight:700}}>✓</span>}</div>)}
