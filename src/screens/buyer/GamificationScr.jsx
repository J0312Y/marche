...
function GamificationScr({onBack}){
  ...
  return(<div className="scr" style={{padding:16,paddingBottom:20}}>
    <div className="appbar" style={{padding:0,marginBottom:12}}><button onClick={onBack}>←</button><h2><Icon name="activity" size={16}/>{" "}Récompenses</h2><div style={{width:38}}/></div>
    ...
    <div style={{padding:16,background:"var(--card)",border:"1px solid var(--border)",borderRadius:16,marginBottom:14}}>
      ...
      <div style={{fontSize:10,color:"var(--muted)",marginTop:6,textAlign:"center"}}>7 jours consécutifs = Bonus 500 pts <Icon name="gift" size={16}/></div>
    </div>
    ...
    <div style={{padding:16,background:"var(--card)",border:"1px solid var(--border)",borderRadius:16,marginBottom:14,textAlign:"center"}}>
      ...
      <div style={{position:"relative",width:180,height:180,margin:"0 auto 14px"}}>
        ...
        <div style={{position:"absolute",top:"50%",left:"50%",transform:"translate(-50%,-50%)",width:50,height:50,borderRadius:"50%",background:"var(--card)",border:"3px solid var(--border)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:20,boxShadow:"0 2px 8px rgba(0,0,0,.1)"}}><Icon name="target" size={18}/></div>
        <div style={{position:"absolute",top:-8,left:"50%",transform:"translateX(-50%)",fontSize:20}}>▼</div>
      </div>
      {prize?<div style={{fontSize:16,fontWeight:700,color:"#F97316"}}><Icon name="sparkle" size={16}/>{" "}{prize}</div>
