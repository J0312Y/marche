...
function LanguageScr({onBack}){
  ...
  return(<div className="scr" style={{padding:16}}><div className="appbar" style={{padding:0,marginBottom:12}}><button onClick={onBack}>←</button><h2>Langue</h2><div style={{width:38}}/></div>
    ...
    <div className="bottom-action-bar"><button className="btn-primary" onClick={()=>{ctxSetLang(lang);toast.success(lang==="fr"?"Langue mise à jour : Français ":lang==="en"?"Language updated: English ":lang==="ln"?"Monoko ebongwani: Lingala ":"Ndinga ebaluki: Kikongo ");onBack()}}>Enregistrer</button></div>
  </div>);
}
