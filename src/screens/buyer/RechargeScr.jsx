...
function RechargeScr({onBack}){
  ...
  return(<><div className="appbar"><button onClick={onBack}>←</button><h2>Recharger Kolo Pay</h2><div style={{width:38}}/></div>
    <div className="scr" style={{padding:16}}>
      ...
      <div style={{paddingTop:24,paddingBottom:16}}><button className="btn-primary" style={{background:amount&&method?"#6366F1":"#E8E6E1",color:amount&&method?"var(--card)":"#908C82"}} onClick={()=>{if(amount&&method){setDone(true);toast.success("Recharge effectuée 💰")}}}>Recharger {amount?fmt(parseInt(amount)):""}</button></div>
    </div>
  </>);
}
